...
ft = np.absolute(np.fft.fft(np.array(datatbft)))
meanft = np.mean(ft)
freq = len(datatbft)/52

testperiod = np.array([i for i in range(2,len(datatbft)+1)])
testfreq = len(datatbft) / testperiod
size = []
for test in testfreq:
    multiples = []
    multiple = test
    while multiple < len(datatbft):
        multiples.append(multiple)
        multiple = multiple + test
    interps = np.interp(multiples, [i for i in range(0,len(datatbft))], ft)
    size.append(np.sum(interps-meanft))

pyplot.plot(testperiod, size)
pyplot.plot([52, 52], [-4,6],'r')
pyplot.xlabel('Period')
pyplot.ylabel('Sum of associated frequency amplitudes (mean subtracted)')
